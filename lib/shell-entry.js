...
(async()=>{const loaded=await P.load();if(!loaded)P.init();if(typeof P.updateSystem==='function')await P.updateSystem();window.POSIXLabsMobile?.init?.();S.out('POSIX.js '+P.VERSION);S.prompt();I.focus()})().catch(e=>S.out('shell: startup failed: '+e.message));
})();
