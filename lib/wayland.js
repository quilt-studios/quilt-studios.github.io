...
class Client{
 ...
 handle(msg){if(msg.length<8)return;let id=u32(msg,0),word=u32(msg,4),size=word>>>16,op=word&0xffff;if(size<8||size>msg.length)return;let p=msg.slice(8,size),obj=this.objects.get(id);if(!obj)return;
  if(obj.iface==='wl_display'){
   ...
   if(op===1){let bid=u32(p,0);let s=this.surfaces.get(id);if(s)s.buffer=bid}
   else if(op===6){window.dispatchEvent(new CustomEvent('posix-wayland-surface',{detail:{type:'commit',id,surface:this.surfaces.get(id),client:this}}))}
...
const bridge={version:'0.2',clients:new Set,endpoint:{connect(sock,proc){let c=new Client(sock,proc);sock.waylandClient=c;bridge.clients.add(c)},receive(bytes,sock,proc){let c=sock.waylandClient;if(!c){c=new Client(sock,proc);sock.waylandClient=c;bridge.clients.add(c)}let o=0;while(o+8<=bytes.length){let size=u32(bytes,o+4)>>>16;if(size<8||o+size>bytes.length)break;c.handle(bytes.slice(o,o+size));o+=size}}}};
