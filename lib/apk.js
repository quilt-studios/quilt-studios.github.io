(()=>{
  ...
  function gzipStarts(bytes) {
    const starts = [];
    for (let i = 0; i + 2 < bytes.length; i++) {
      if (bytes[i] === 0x1f && bytes[i + 1] === 0x8b && bytes[i + 2] === 0x08) starts.push(i);
    }
    return starts;
  }
  ...
  async function gunzip(bytes, label, legacy = false) {
    ...
    if (bytes.length < 3 || bytes[0] !== 0x1f || bytes[1] !== 0x8b || bytes[2] !== 0x08) throw fail('EEXTRACT', 'package is not a gzip stream');
    UI()?.show('Extracting ' + label, legacy ? 'Reading Alpine APK gzip members' : 'Decompressing package', 5);
    ...
    // Alpine v2 APK files are a concatenation of independently compressed
    // gzip members (signature, control metadata and data payload). Browser
    // DecompressionStream implementations such as WebKit reject trailing
    // members with "Extra bytes past the end", so decode one member at a time.
    const starts = gzipStarts(bytes);
    if (!starts.length || starts[0] !== 0) throw fail('EEXTRACT', 'invalid Alpine APK gzip layout');
    ...
    while (startIndex < starts.length) {
      ...
      // A gzip magic sequence can occur by chance inside compressed data.
      // Try candidate boundaries until DecompressionStream accepts a complete
      // member. This avoids depending on non-standard inflater APIs.
      for (let j = startIndex + 1; j <= starts.length; j++) {
        const end = j < starts.length ? starts[j] : bytes.length;
        if (end - start < 18) continue;
        try {
          decoded = await inflateGzip(bytes.slice(start, end));
          chosenEnd = end;
          chosenIndex = j;
          break;
        } catch (_) {}
      ...
      if (!decoded) throw fail('EEXTRACT', 'could not decode Alpine APK gzip member at byte ' + start);
      if (decoded.length) members.push(decoded);
      UI()?.set(10 + Math.floor((chosenEnd / bytes.length) * 60), 'Decoded APK member ' + members.length);
      startIndex = chosenIndex;
    }

    if (!members.length) throw fail('EEXTRACT', 'Alpine APK contained no decodable gzip members');
    const total = members.reduce((n, member) => n + member.length, 0);
    ...
    UI()?.set(70, 'Decompressed ' + members.length + ' APK members · ' + P.fmt(result.length));
    return result;
  }
