(()=>{
  ...
  async function gunzipSingle(bytes, label, out) {
    if (typeof DecompressionStream === 'undefined') {
      throw fail('EUNSUPPORTED', 'this browser does not support gzip DecompressionStream');
    }
    UI()?.show('Extracting ' + label, 'Decompressing package', 5);
    let buffer;
    try {
    ...
    out('extract ' + label + ' · ' + P.fmt(result.length));
    return result;
  }

  async function unpack(raw, file, label, out) {
    if (!raw?.length) throw fail('EDOWNLOAD', 'downloaded package is empty');
    if (!file.endsWith('.browser.tar.gz')) {
      throw fail('EMIRROR', 'package mirror is still using legacy .apk format; wait for the Alpine mirror rebuild and run apk update');
    }
    return gunzipSingle(raw, label, out);
  }
  ...
  async function untar(bytes, label) {
    ...
    if (!files.length) throw fail('EARCHIVE', 'archive contained no installable files');
  ...
  async function catalog() {
    ...
    let text;
    try {
      response = await P.netFetch('/alpine/packages.tsv');
  ...
  async function install(packageName, out) {
    if (!packageName || !/^[A-Za-z0-9+_.-]+$/.test(packageName)) {
      throw fail('EINVAL', 'invalid package name: ' + String(packageName || ''));
    }
    ...
    try {
      ...
      let download;
      try {
        download = await P.download('/alpine/' + pkg.repo + '/riscv64/' + pkg.file, progress(out, packageName, 'Downloading'));
      } catch (error) {
        throw fail('EDOWNLOAD', 'failed to download ' + packageName + ': ' + error.message);
      }

      const raw = download.bytes;
      out(packageName + ' ' + bar(raw.length, raw.length));

      const payload = await unpack(raw, pkg.file, packageName, out);
      UI()?.set(70, 'Parsing archive');
      const files = await untar(payload, packageName);

      out('installing ' + packageName + ' · ' + files.length + ' entries');
      ...
      UI()?.fail((error.code ? '[' + error.code + '] ' : '') + error.message);
      throw error;
    }
  }
  ...
  P.apk = async (args, out) => {
    ...
    try {
      if (command === 'help') {
        out('apk: update | add <pkg...> | search <query> | list --installed | info <pkg> | arch');
        return;
      }
      ...
      if (command === 'update') {
        ...
        for (const repo of ['main', 'community']) {
          try {
            ...
            const download = await P.download('/alpine/' + repo + '/riscv64/APKINDEX.tar.gz', progress(out, repo, 'Updating index'));
            const bytes = download.bytes;
            if (!bytes.length) throw fail('EINDEX', repo + ' index is empty');
            P.write('/var/cache/apk/' + repo + '.tar.gz', bytes, true);
            UI()?.done(repo + ' index updated');
            out('OK ' + repo + ' · ' + P.fmt(bytes.length));
          } catch (error) {
            errors.push(repo + ': ' + error.message);
            UI()?.fail(repo + ': ' + error.message);
        ...
        try {
          ...
          throw fail('EDB', 'could not save package indexes: ' + error.message);
        }
      ...
      if (command === 'search') {
        ...
        const catalogMap = await catalog();
        let count = 0;
        for (const [name, info] of catalogMap) {
      ...
      if (command === 'info') {
        ...
        const packageName = a[0];
        const info = P.packages.get(packageName);
        if (!info) throw fail('ENOENT', packageName + ' is not installed');
        out(packageName + '-' + info.v + '\nrepository: ' + info.repo + '\nfiles: ' + (info.files ?? 'unknown'));
        return;
      }

      throw fail('ECOMMAND', 'unknown apk command: ' + command);
    } catch (error) {
      const message = 'apk: ' + (error.code ? '[' + error.code + '] ' : '') + error.message;
      UI()?.fail(message);
